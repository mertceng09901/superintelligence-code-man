...
exports.createOrder = async (req, res) => {
    try {
        ...
        const orderItems = cart.items.map(item => ({ product: item.product._id, quantity: item.quantity, selectedColor: item.selectedColor || 'Varsayılan', priceAtPurchase: item.product.price }));
        ...
        // Redis cache'i temizle (yeni sipariş oluştuğunda)
        const redisClient = getRedisClient();
        if (redisClient) {
            try {
                await redisClient.del('products_cache');
            } catch (e) {
                // Redis hatası siparişi engellemesin
            }
        }
