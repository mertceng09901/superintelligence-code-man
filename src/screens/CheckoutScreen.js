...
export default function CheckoutScreen({ navigation }) {
  ...
  const fetchSummary = async () => {
    try {
      ...
      Alert.alert('Hata', 'Özet yüklenemedi.');
    }
        ...
        >
          ...
          <View style={[s.card, SHADOWS.small]}>
            ...
            <View style={s.totalRow}>
              <Text style={s.totalLbl}>Toplam</Text>
              <Text style={s.totalVal}>{fmt(summary?.totalAmount || 0)} ₺</Text>
            </View>
          </View>
          ...
          <View style={[s.card, SHADOWS.small]}>
            <View style={s.cardHead}>
              ...
              <Text style={s.cardTitle}>Teslimat Adresi</Text>
            </View>
            <TextInput
              style={s.addrInput}
          ...
          <View style={[s.card, SHADOWS.small]}>
            <View style={s.cardHead}>
              ...
              <Text style={s.cardTitle}>Ödeme Yöntemi</Text>
            </View>
            {[
              { id: 'CREDIT_CARD', lbl: 'Kredi Kartı', ic: 'card-outline' },
              { id: 'CASH', lbl: 'Kapıda Ödeme', ic: 'cash-outline' }
            ].map(o => (
              <View key={o.id}>
                <TouchableOpacity
                  style={[s.payOpt, paymentMethod === o.id && s.payOptActive]}
                  onPress={() => setPaymentMethod(o.id)}
                >
                  ...
                  <View style={[s.radio, paymentMethod === o.id && s.radioA]}>
                    {paymentMethod === o.id && <View style={s.radioDot} />}
                  </View>
                </TouchableOpacity>
                ...
                {o.id === 'CREDIT_CARD' && paymentMethod === 'CREDIT_CARD' && (
                  <View style={s.cardForm}>
                    ...
                    <TextInput style={s.cardInput} placeholder="Kart Numarası (16 hane)" placeholderTextColor={COLORS.textMuted} value={cardNumber} onChangeText={handleCardNumberChange} keyboardType="numeric" maxLength={19} />
          ...
          {/* Sipariş Butonu — ScrollView içinde ama en altta belirgin */}
          <TouchableOpacity onPress={handlePlaceOrder} disabled={placing} activeOpacity={0.85}>
            <LinearGradient colors={COLORS.gradientSecondary} style={s.orderBtn} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}>
              {placing
                ? <ActivityIndicator color="#FFF" />
                : <>
                    <Ionicons name="checkmark-circle" size={22} color="#FFF" />
                    <Text style={s.orderTxt}>Siparişi Onayla — {fmt(summary?.totalAmount || 0)} ₺</Text>
                  </>
              }
            </LinearGradient>
          </TouchableOpacity>
...
const s = StyleSheet.create({
  ...
  payOpt: {
    ...
    padding: 14, borderRadius: SIZES.radiusSm, marginBottom: 8,
  ...
  cardForm: {
    backgroundColor: 'rgba(108,99,255,0.03)', padding: 12,
    borderRadius: SIZES.radiusSm, borderWidth: 1,
    borderColor: 'rgba(108,99,255,0.1)', marginBottom: 8, marginTop: -4,
  },
  cardInput: {
    backgroundColor: '#FFF', borderRadius: SIZES.radiusSm,
  ...
  infoBox: {
    ...
    backgroundColor: 'rgba(59,130,246,0.08)', borderRadius: SIZES.radiusSm,
    padding: 14, marginBottom: 16,
  },
  infoTxt: { flex: 1, fontSize: 12, color: COLORS.info, lineHeight: 18 },
  orderBtn: {
    flexDirection: 'row', alignItems: 'center', justifyContent: 'center',
    borderRadius: SIZES.radiusSm, paddingVertical: 18, gap: 10,
    marginBottom: 8,
  },
  orderTxt: { color: '#FFF', fontSize: 17, fontWeight: '700' },
});
